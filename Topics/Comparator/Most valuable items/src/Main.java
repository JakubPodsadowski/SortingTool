...
class Utils {
    public static List<StockItem> sort(List<StockItem> stockItems) {
        List<StockItem> sortedStockItems = new ArrayList<>(stockItems);
        sortedStockItems.sort(Comparator.comparingDouble((StockItem a) -> a.getPricePerUnit() * a.getQuantity()).reversed());
